/**
 * Aplica la migración 20260527_mifact_baseurl_api (Mifact baseUrl → raíz del API JSON con /api)
 * a todos los schemas tenant_* (o solo al tenant indicado con --tenant <name>).
 *
 *   Uso: pnpm exec tsx scripts/aplicar-migracion-mifact-baseurl.ts
 *        pnpm exec tsx scripts/aplicar-migracion-mifact-baseurl.ts --tenant tenant_mi_tienda
 */
...
async function main() {
  ...
  console.log('');
  console.log(`✅ Mifact baseUrl /api: migración completada.`);
