...
import { ReporteClientesService, FiltrosReporteClientes } from './reporte-clientes.service';
...
export class ReporteClientesController {
  constructor(private readonly service: ReporteClientesService) {}

  @Get('excel') @RequierePermiso('clientes:leer')
  async excel(@Query() q: FiltrosReporteClientes, @Tenant() ctx: TenantContext, @Res() res: Response) {
  ...
  @Get('pdf') @RequierePermiso('clientes:leer')
  async pdf(@Query() q: FiltrosReporteClientes, @Tenant() ctx: TenantContext, @Res() res: Response) {
...
function nombre(ext: string): string {
  return `reporte-clientes-${new Date().toISOString().slice(0, 10)}.${ext}`;
}
